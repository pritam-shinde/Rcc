        ...
        try {
            ...
            if (checkUser) {
                if (matchPass) {
                    ...
                    res.status(400).json({ error: { errorMessage: ["Password does'nt match"] } })
                }
            }else{
                res.status(400).json({error: {errorMessage: ["Your email does not exist...!"]}})
            }
        } catch (error) {
...
module.exports.userLogout = (req, res) => {
    res.status(201).cookie('authToken', "").json({ successMessage: true })
}