    ...
    return (
        <>
            <ListItem className={classes.ListItem}>
                <Box className={classes.listBox} onClick={() => { getCurrentFriend(friendInfo); }} style={{ backgroundColor: friendInfo._id === currentFriend._id ? "#066fac" : "transparent" }}>
                    <Badge>
                        <Avatar src={`./image/${friendInfo.image}`} alt={friend.username} style={{ height: "1.5rem", width: "1.5rem", marginRight: "0.5rem" }} />
                    </Badge>
                    <h5 style={{ fontSize: "0.9375rem", fontWeight: 400, cursor: "pointer", color: friendInfo._id === currentFriend._id ? "#fff" : "#6c757d" }} > {friendInfo.username}</h5>
                </Box>
            </ListItem>
        </>
    )
