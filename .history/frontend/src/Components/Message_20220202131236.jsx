...
const Message = ({ message, scrollRef }) => {
    const convertTime = () => {
        if (message.message.text !== "") {
            ...
            return <p className={localStorage.getItem('isDark') ? "text-white" : "text-dark"}>{sentAtTimeDate.getMonth() < 10 ? `0${sentAtTimeDate.getMonth()}` : sentAtTimeDate.getMonth()}/{sentAtTimeDate.getDate() < 10 ? `0${sentAtTimeDate.getDate()}` : sentAtTimeDate.getDate()
                ...
                <div className="media" style={{ width: "100%" }} ref={scrollRef} >
                    ...
                    <div className="media-body">
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div>
                                <h5 className={`${localStorage.getItem('isDark') ? "text-white" : "text-dark"} mt-0` } style={{ fontSize: "0.9375rem", fontWeight: 700, color: "#121212", lineHeight: "1.5rem" }}>{message.senderName}</h5>
                            </div>
                            <div>{convertTime()}</div>
                        </div>
                        <div style={{ width: message.message.text !== "" ? "100%" : "30%" }}>
                            {message.message.text !== "" ? <p style={{ fontSize: "0.9375rem", fontWeight: 400, lineHeight: "1.5rem" }} className={localStorage.getItem('isDark') ? "text-white" : "text-muted" }>{message.message.text}</p> : <img src={`./image/${message.message.image}`} alt="message-attachment" className='img-fluid' />}
                        </div>
                    </div>
                </div>
            </ListItem>
