            ...
            <Box px={1} py={2} className={classes.root}>
                ...
                <Box mt={2}>
                    <Box className='d-flex justify-content-between align-items-center'>
                        ...
                        <IconButton onClick={() => setState(!state)}>{!state ? <RiArrowDropDownLine style={{color: localStorage.getItem('isDark') ? classes.darkIcon : classes.icon}} /> : <RiArrowDropUpLine className={localStorage.getItem('isDark') ? classes.darkIcon : classes.icon} />}</IconButton>
                    </Box>
                    <Divider />
                    <Collapse in={state}>
