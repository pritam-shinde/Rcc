            ...
            <Box px={1} py={2} className={classes.root}>
                ...
                <Box mt={2}>
                    <Box className='d-flex justify-content-between align-items-center'>
                        ...
                        <IconButton onClick={() => setOn(!on)}>{!on ? <RiArrowDropDownLine className={localStorage.getItem('isDark') ? classes.darkIcon : classes.icon} /> : <RiArrowDropUpLine style={{color: localStorage.getItem('isDark') ? classes.darkIcon : classes.icon}} />}</IconButton>
                    </Box>
                    <Divider />
                    <Collapse in={on}>
                        <Box className='d-flex overflow-scroll mb-3 py-3'>
