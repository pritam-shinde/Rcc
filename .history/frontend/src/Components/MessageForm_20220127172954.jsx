...
const MessageForm = ({ sendMessage, imageSend, isTyping }) => {
                ...
                <Box py={2} px={2}>
                    <form onSubmit={(e) => { e.preventDefault(); sendMessage(message); setMessage("");  }}>
                        <Box className={classes.root}>
                            <input type="text" placeholder='Message' className='form-control' multiple onChange={(e) => {setMessage(e.target.value); isTyping(true)}} value={message} />
                    ...
                    <Box mt={1}>
                        <small className="text-muted" style={{fontSize:"0.7535rem"}}>Ankit is typing...</small>
                    </Box>
                </Box>
            </Box>
        </>
