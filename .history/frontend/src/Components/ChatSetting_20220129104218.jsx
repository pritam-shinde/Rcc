...
const ChatSetting = ({ friends, currentFriend, activeUser, message, myInfo }) => {
    ...
    let imageMessage = message.filter(message => {
        return message.message.text === "" && (message.message.senderId === myInfo.id && message.message.receiverId === currentFriend._id || message.message.senderId === currentFriend._id && message.message.receiverId === myInfo.id) && message.message.image !== ""
    })
